...
class OrderForm(forms.ModelForm):
    ...
    quantity = forms.IntegerField(
        label=u'* Cantidad de Producto:',
        required=True,
        widget=forms.TextInput(
            attrs={'class': 'form required', 'placeholder': 'Ingrese un número'}
        ),
        error_messages={
            'required': u'Este campo es requerido y no se puede dejar vacío.',
            'invalid': u'Debe ingresar un número entero.',
        }
    )
    ...
    class Meta:
        model = Order
        fields = ("quantity", "contact_name", "contact_email", "contact_phone", "contact_message")